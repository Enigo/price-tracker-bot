...
async def handle_check_button(
    ...
    text = _("✅ <b>#{product_id}</b> {name}\n💰 Price: {price}").format(
        product_id=product_id, name=_escape_html(name), price=price_str
    )
    if initial and current and initial > 0 and initial != current:
        diff = (initial - current) / initial * 100
        if diff > 0:
            text += _(
                "\n📌 Initial: €{initial:.2f} (<i>-{diff:.1f}&#37; since tracking</i>)"
...
async def handle_chart_button(
    ...
    if chart:
        ...
        await query.message.reply_text(
            _("📭 Not enough data to generate the chart (at least 2 points are required).")
        )
    return True
        ...
        if data.startswith(prefix):
            ...
            await query.edit_message_text(
                _(
                    ...
                    "📦 {name}\n\n"
                    "<b>How would you like to be notified?</b>"
                ).format(
                    preference=_(label),
...
async def handle_track_choice(
    ...
    if data.startswith("track_any_"):
        ...
        await query.edit_message_text(
            _(
                "🔔 <b>Every price drop</b> enabled for #{product_id}\n"
                "📦 {name}\n\n"
                "You will receive a notification for every price drop."
    ...
    if data.startswith("track_threshold_"):
        ...
        await query.edit_message_text(
            _(
                "📉 <b>Set threshold for #{product_id}</b>\n"
                "📦 {name}\n\n"
                "Enter the desired threshold:\n"
                "• <code>20%</code> — notify me if it drops by 20%\n"
                "• <code>50</code> — notify me if it drops by €50"
            ).format(product_id=product_id, name=_escape_html(name)),
            parse_mode=ParseMode.HTML,
        )
        return True
    ...
    if data.startswith("track_target_"):
        ...
        await query.edit_message_text(
            _(
                "💰 <b>Set target price for #{product_id}</b>\n"
                "📦 {name}{price_hint}\n\n"
                "Enter the target price (e.g. <code>100</code>):"
            ).format(
                product_id=product_id,
                name=_escape_html(name),
    ...
    if data.startswith("track_default_"):
        ...
        await query.edit_message_text(
            _(
                "👍 <b>Default -10% threshold</b> for #{product_id}\n"
                "📦 {name}\n\n"
                "You will receive a notification when the price drops 10&#37; "
