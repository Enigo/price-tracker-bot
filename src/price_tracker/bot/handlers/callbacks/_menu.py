...
async def handle_menu_navigation(
    ...
    if data == "menu_main":
        ...
        rows = [
            ...
            [InlineKeyboardButton(_("🔍 Check prices"), callback_data="menu_prezzi")],
            ...
            [InlineKeyboardButton(_("📊 Info and statistics"), callback_data="menu_info")],
        ]
        if is_admin:
    ...
    if data == "menu_storia":
        ...
        rows.append(menu_back_button())
        await query.edit_message_text(
            _("📊 <b>Price history</b>\n\nTap a product to view its chart."),
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(rows),
        )
        return True
    ...
    if data == "menu_importa_info":
        await query.edit_message_text(
            _(
                "📂 <b>Import products</b>\n\n"
                "Send a CSV file in chat (created with Export).\n"
...
async def _handle_menu_info(
    ...
    if is_admin:
        ...
        text += _(
            ...
            users=len(users),
            products=gs["active_products"],
            checks=gs["total_checks"],
        )
    await query.edit_message_text(
        text,
        parse_mode=ParseMode.HTML,
