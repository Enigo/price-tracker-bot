...
async def cmd_set_interval(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if minutes >= 60:
        ...
        display = ngettext("{n} hour", "{n} hours", 1 if hours == 1 else 2).format(n=hours_str)
    else:
        display = ngettext("{n} minute", "{n} minutes", minutes).format(n=minutes)
    await update.message.reply_text(
        _("✅ Interval updated: <b>every {interval}</b>").format(interval=display),
        parse_mode=ParseMode.HTML,
    )
...
async def mute_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if product_id is None and mute_until is None:
        message = _("Muted all products forever.")
    elif product_id is None:
        message = _("Muted all products until {date}.").format(date=mute_until.isoformat())
    elif mute_until is None:
        message = _("Muted product {product_id} forever.").format(product_id=product_id)
    else:
        message = _("Muted product {product_id} until {date}.").format(
            product_id=product_id, date=mute_until.isoformat()
        )
...
async def digest_mode_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await repo.upsert_notification_prefs(prefs)
    if enabled:
        message = _("Digest mode on (interval {interval}m).").format(interval=interval)
    else:
        message = _("Digest mode off.")
    await update.message.reply_text(message)
...
async def timezone_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if tz not in _VALID_TIMEZONES:
        await update.message.reply_text(_("Unknown timezone: {timezone}").format(timezone=tz))
        return
    user_id = update.effective_user.id
    existing = await repo.get_notification_prefs(user_id=user_id, product_id=None)
    if existing is not None:
    ...
    await repo.upsert_notification_prefs(prefs)
    await update.message.reply_text(_("Timezone set to {timezone}.").format(timezone=tz))
...
async def prefs_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    scope = _("product {product_id}").format(product_id=product_id) if product_id else _("global")
    await update.message.reply_html(
        _(
            "<b>Effective preferences</b>\n"
            "  scope: {scope}\n"
            "  mute: {mute}\n"
            "  digest_mode: {digest} (interval {interval}m)\n"
            ...
            scope=scope,
            mute=eff.mute,
            digest=eff.digest_mode,
            interval=eff.digest_interval_minutes,
            quiet_start=eff.quiet_hours_start or "—",
            quiet_end=eff.quiet_hours_end or "—",
            throttle=eff.throttle_per_hour or _("unlimited"),
