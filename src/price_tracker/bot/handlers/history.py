...
async def cmd_history(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        ...
        await update.message.reply_text(
            _("📊 <b>Choose a product for its price history:</b>"),
    ...
    if chart_buf:
        ...
        if lowest:
            caption += _("\n📉 Lowest: €{price:.2f}").format(price=lowest)
        if highest:
            caption += _("  📈 Highest: €{price:.2f}").format(price=highest)
    ...
    if not context.args:
        await update.message.reply_text(
            _(
                ...
                "Useful when the price has dropped and you want to restart the comparison."
            ),
            parse_mode=ParseMode.HTML,
        )
        return
