...
from price_tracker.bot.messages import _, localize_error
...
async def cmd_delete(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        ).format(product_id=product_id, name=_escape_html(name[:80])),
        parse_mode=ParseMode.HTML,
...
async def cmd_target(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await _product_picker(
            ...
            context,
            "target",
            _("Choose a product to set a target"),
            "settarget",
        )
        return
    ...
    if current:
        ...
        if current <= target:
            ...
            lines.append(
                _("💰 Current: {price} (-{percent:.1f}% needed)").format(
...
async def cmd_threshold(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await _product_picker(
            ...
            context,
            "threshold",
            _("Choose a product to set a threshold"),
            "setsoglia",
        )
        return
    if len(context.args) < 2:
        await update.message.reply_text(
            _(
                ...
                "<code>/threshold 3 any</code> — notify me on every price drop"
            ),
            parse_mode=ParseMode.HTML,
        )
        return
...
async def _add_product(
    ...
    if existing:
        ...
        if is_active:
            ...
            await update.message.reply_text(
                _("ℹ️ You are already tracking this product (#{product_id}).{price}").format(
                    product_id=existing["id"], price=price_str
                )
            )
            return
    ...
    try:
        ...
        await msg.edit_text(
            _("🛡 The site is currently blocking automated requests.\nTry again in a few minutes.")
        )
        return

    if result.price is None:
        error_msg = localize_error(result.error) if result.error else _("Price not found")
        await msg.edit_text(
            _(
