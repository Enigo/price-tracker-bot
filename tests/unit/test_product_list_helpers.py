...
def test_relative_time_naive_timestamp_is_treated_as_utc():
    ...
    assert _format_relative_time("2026-06-15 16:00:00", now=now) == "30 min ago"


def test_relative_time_hours_and_days():
    now = datetime(2026, 6, 15, 16, 0, tzinfo=UTC)
    assert _format_relative_time("2026-06-15 13:00:00", now=now) == "3h ago"
    assert _format_relative_time("2026-06-13 16:00:00", now=now) == "2d ago"


def test_relative_time_aware_timestamp_with_z():
    now = datetime(2026, 6, 15, 16, 30, tzinfo=UTC)
    assert _format_relative_time("2026-06-15T16:00:00Z", now=now) == "30 min ago"
